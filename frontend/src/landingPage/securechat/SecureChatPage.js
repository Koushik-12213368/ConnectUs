...
function SecureChatPage() {
  ...
  const chatDirectoryLabel = useMemo(() => {
    ...
    return myRoleIsDoctor ? "Patients" : "Doctors";
  }, [me, myRoleIsDoctor]);
  ...
  const sendMessage = async (e) => {
    ...
    if (!me) {
      setError("Please login first to send messages.");
      return;
    }
    if (!activeRoom) {
      setError("Please select a chat room before sending a message.");
      return;
    }
            ...
            <div className="card-body">
              ...
              <div className="d-flex flex-column gap-2">
                {directory.length > 0 ? (
                  ...
                  <div className="small text-muted p-3 rounded border bg-light">
                    {isAdmin ? (
                      <>
                        Secure chat is only available for doctor and patient
                        accounts. Please log in as a doctor or patient to start a
                        secure chat.
                      </>
                    ) : (
                      <>
                        No {chatDirectoryLabel.toLowerCase()} available.
                        {myRoleIsDoctor
                          ? " Ask a patient to sign up so you can start a secure chat."
                          : " Ask a doctor to sign up so you can start a secure chat."}
                      </>
                    )}
                  </div>
                )}
            ...
            <div className="card-body d-flex flex-column" style={{ minHeight: 520 }}>
              ...
              <form onSubmit={sendMessage} className="mt-3 d-flex gap-2">
                ...
                <button
                  ...
                  type="submit"
                  disabled={loading || !draft.trim() || !activeRoom}
                >
